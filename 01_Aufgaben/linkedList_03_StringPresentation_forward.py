...
class Liste:
    ...
    def __repr__(self):
        def repr_rekursiv(wagon: Wagon, values):
            if wagon.next is None:
                return f'[{values}{repr(wagon.value)}]'
            return repr_rekursiv(wagon.next, values + f'{repr(wagon.value)}, ')

        values = ''
        return '[]' if self.first is None else repr_rekursiv(self.first, values)
